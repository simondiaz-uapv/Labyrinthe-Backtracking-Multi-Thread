...
#include "cursor.cpp"
#include <locale>
#include <codecvt> // Pour les conversions UTF-8 (si nécessaire)
...
int main() {
    setlocale(LC_ALL, ""); // Utilise la locale par défaut du système
    wcout.imbue(locale("")); // Configure cout pour UTF-8

    string cheminFichier = "labyrinthe.txt";
    vector<Labyrinthe> labyrinthes = chargerLabyrinthes(cheminFichier);

    wcout << u8"Nombre de labyrinthes chargés : " << labyrinthes.size() << endl;
